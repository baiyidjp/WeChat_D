...
- (void)stopRecord
{
    ...
    if (cTime > 1) {
        [self audio_PCMtoMP3WithTime:cTime];
    }else {
...
- (void)audio_PCMtoMP3WithTime:(NSInteger)recordTime
{
    ...
    @finally {
        ...
        NSLog(@"MP3转换结束");
        if (_delegate && [_delegate respondsToSelector:@selector(endConvertWithMP3FileName:recordTime:)]) {
            [_delegate endConvertWithMP3FileName:mp3FilePath recordTime:recordTime];
        }
        [self deleteCafCache];
    }
...
- (NSString *)mp3Path {
    NSString *mp3Path = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"voiceCache"];
